...
namespace DF_FaceTracking.cs
{
   ...
   //             moduleConfiguration.ApplyChanges();
        ...
        //private void Savedata_Acess(PXCMFaceData faceOutput)
        //{
        //    ////zz
        //    PXCMFaceData.Face qface = faceOutput.QueryFaceByIndex(0);
        //    PXCMFaceData.PoseData posedata = qface.QueryPose();
        ...
        //    string connSting;//数据库连接信息
        //    OleDbConnection OleDbConn;//Acess数据库连接对象
        //    OleDbDataAdapter OleDbAda;//数据库连接对象    可以直接和DataSet联系，并操作数据源的，它的功能相对强大一些，因此也比较耗系统资源！
        //    //OleDbDataReader 依次向前读取，显示数据 资源消耗相对较小

        //    connSting = "provider=Microsoft.ACE.OLEDB.12.0; Data Source=" + System.IO.Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location)+ "\\DB.accdb";

        //    OleDbConn = new OleDbConnection(connSting);
        //    try
        //    {
        //        OleDbConn.Open();
        //    }
        ...
        //        sql_Insert = "insert into surprise([time],[LandmarkPointName],[Positionworld_x],[Positionworld_y],[Positionworld_z],[PositionImage_x],[PositionImage_y],[HeadCenter_x],[HeadCenter_y],[HeadCenter_z],[PoseEulerAngles_pitch],[PoseEulerAngles_roll],[PoseEulerAngles_yaw])values('"
        //            + times + "','"
        //            + LandmarkPointName + "','"
        //            + Positionworld_x + "','"
        //            + Positionworld_y + "','"
        ...
        //        OleDbCommand sCmd = new OleDbCommand(sql_Insert, OleDbConn);
        //        sCmd.ExecuteNonQuery();
