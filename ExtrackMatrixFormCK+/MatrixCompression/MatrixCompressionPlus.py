...
def ExtracMatrix(dir,matrixDir):
    ...
    matrixCp = [(['0'] * 98) for i in range(66)]
    file_txt=open(dir,"r")
    ...
    for i in range(0,98):
        for j in range(0,66):
            matrix_file.write(matrixCp[j][i])
        matrix_file.write("\n")

    matrix_file.close()
    print "CpPlus________"+str(matrixcount)
...
def readFile(filepath):
    ...
    MatrixDir = os.path.join(nowDir, "MatrixCpPlus_"+maxfileName)
